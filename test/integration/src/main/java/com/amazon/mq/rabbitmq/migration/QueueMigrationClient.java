...
/**
 * Client for RabbitMQ Queue Migration API endpoints. Handles /api/queue-migration REST calls using
 * plain Java HTTP client.
 */
public class QueueMigrationClient {
  ...
  public MigrationResponse startMigration(boolean skipUnsuitableQueues)
      throws IOException, InterruptedException {
    return startMigration(skipUnsuitableQueues, null, null);
  }

  /** Start queue migration with batch options */
  public MigrationResponse startMigration(Integer batchSize, String batchOrder)
      throws IOException, InterruptedException {
  ...
  public MigrationResponse startMigration(
    ...
    try {
      ...
      if (msg != null && msg.contains("EOF")) {
        throw new IOException(
            "Migration start failed (likely unsuitable queues or validation error)", e);
      }
    ...
    if (statusCode != 200 && statusCode != 202 && statusCode != 204) {
      throw new IOException(
          "Failed to start migration. Status: " + statusCode + ", Body: " + responseBody);
    }
  ...
  public void interruptMigration(String migrationId) throws IOException, InterruptedException {
    ...
    if (response.statusCode() != 200
      ...
      throw new IOException(
          "Failed to interrupt migration. Status: "
              + response.statusCode()
              + ", Body: "
              + response.body());
    }
  ...
  public MigrationDetailResponse getMigrationDetails(String migrationId)
      throws IOException, InterruptedException {
    HttpRequest request =
        HttpRequest.newBuilder()
    ...
    if (response.statusCode() != 200) {
      throw new IOException(
          "Failed to get migration details. Status: "
              + response.statusCode()
              + ", Body: "
              + response.body());
    }
  ...
  private MigrationDetailResponse parseMigrationDetails(String responseBody) throws IOException {
    ...
    return new MigrationDetailResponse(
        displayId, status, completedQueues, totalQueues, queueStatuses);
  }
  ...
  public MigrationStatusResponse getMigrationStatus() throws IOException, InterruptedException {
    ...
    if (response.statusCode() != 200) {
      throw new IOException(
          "Failed to get migration status. Status: "
              + response.statusCode()
              + ", Body: "
              + response.body());
    }
    ...
    MigrationInfo migrationInfo =
        new MigrationInfo(
            ...
            latestMigration.has("progress_percentage")
                ...
                : 0,
            latestMigration.has("completed_queues")
                ? latestMigration.get("completed_queues").asInt()
                : 0,
            latestMigration.has("total_queues") ? latestMigration.get("total_queues").asInt() : 0,
            latestMigration.has("started_at")
                ? latestMigration.get("started_at").asText()
                : "unknown",
            latestMigration.has("completed_at")
                ? latestMigration.get("completed_at").asText()
                : null);
  ...
  public static class MigrationStatusResponse {
    ...
    public String toString() {
      return String.format(
          "MigrationStatusResponse{overallStatus='%s', migrationInfo=%s}",
  ...
  public static class MigrationInfo {
    ...
    public String toString() {
      return String.format(
          "MigrationInfo{id='%s', status='%s', progress=%d%%, queues=%d/%d, started='%s'}",
  ...
  public static class MigrationDetailResponse {
    ...
    public MigrationDetailResponse(
        String displayId,
        String status,
        int completedQueues,
        int totalQueues,
        java.util.List<QueueMigrationStatus> queueStatuses) {
      this.displayId = displayId;
      this.status = status;
      this.completedQueues = completedQueues;
      this.totalQueues = totalQueues;
    ...
    public java.util.List<QueueMigrationStatus> getQueueStatuses() {
