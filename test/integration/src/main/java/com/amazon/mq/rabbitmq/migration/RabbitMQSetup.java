...
public class RabbitMQSetup {
  ...
  public void initialize()
      ...
      try {
        ...
        connections[i] =
            factory.newConnection(
                "migration-test-setup-" + endpoint.getHostname() + "-" + endpoint.getPort());
        channels[i] = connections[i].createChannel();

        // NOTE: intentionally NOT enabling publisher confirmations

        logger.info(
            "AMQP connection established to {}:{} vhost '{}' with confirmations enabled",
            endpoint.getHostname(),
            endpoint.getPort(),
            endpoint.getVirtualHost());
      } catch (IOException e) {
        logger.error(
            "Failed to connect to AMQP at {}:{}: {}",
            endpoint.getHostname(),
            endpoint.getPort(),
            e.getMessage());
        throw new IOException(
            "AMQP connection failed to " + endpoint.getHostname() + ":" + endpoint.getPort(), e);
      }
    }
    ...
    logger.info(
        "RabbitMQ connections established successfully to {} AMQP hosts and {} HTTP hosts",
        nodeCount,
        nodeCount);
  }
  ...
  private void createExchanges() throws IOException {
    ...
    getNextChannel()
        .exchangeDeclare("test.headers", BuiltinExchangeType.HEADERS, true, false, null);
  ...
  private void createQueues() throws IOException {
    ...
    logger.info(
        "Creating {} mirrored classic queues across {} nodes (round-robin)",
        queueCount,
        connections.length);
    ...
    for (int i = 0; i < queueCount; i++) {
      ...
      logger.debug(
          "Created queue: {} on node {} with arguments: {}", queueName, nodeIndex + 1, arguments);
    }
  ...
  private void createUnsuitableQueues() throws IOException {
    ...
    for (int i = 0; i < unsuitableQueueCount; i++) {
      ...
      switch (unsuitableType) {
        case "reject-publish-dlx":
          ...
          logger.debug(
              "Creating unsuitable queue: {} with x-overflow=reject-publish-dlx", queueName);
          break;

        case "too-many-messages":
          // We'll create this queue and then fill it with too many messages
          arguments.put(
              "x-max-length", maxMessagesPerQueue + 10000); // Allow more than the migration limit
        ...
        case "too-many-bytes":
          ...
          arguments.put(
              "x-max-length-bytes",
              maxBytesPerQueue + (50 * 1024 * 1024)); // Allow more than the migration limit
      ...
      nodeChannel.queueDeclare(queueName, true, false, false, arguments);
      logger.debug(
          "Created unsuitable queue: {} on node {} with type: {}",
          queueName,
          nodeIndex + 1,
          unsuitableType);
    }
    ...
    logger.info(
        "Created {} unsuitable queues for testing migration compatibility checks",
        unsuitableQueueCount);
  ...
  private void fillUnsuitableQueuesWithPublisher(
    ...
    try {
      ...
      List<com.amazon.mq.rabbitmq.publishing.PublishingTask> allTasks =
          generateAllUnsuitableQueueTasks(
      ...
      if (!allTasks.isEmpty()) {
        ...
        logger.info(
            "Successfully filled {} unsuitable queues with {} total messages in {} ms ({} msg/sec)",
            unsuitableQueueCount,
        ...
        if (result.getNackedCount() > 0) {
          logger.info(
              "Note: {} messages were nacked (expected for queues with max-length limits)",
      ...
      logger.error(
          "Failed to fill unsuitable queues with MultiThreadedPublisher: {}", e.getMessage(), e);
  ...
  private List<com.amazon.mq.rabbitmq.publishing.PublishingTask> generateAllUnsuitableQueueTasks(
      int unsuitableQueueCount,
      int maxMessages,
      int maxBytes,
      byte[] smallMessageBody,
      byte[] largeMessageBody) {
    ...
    for (int i = 0; i < unsuitableQueueCount; i++) {
      ...
      if ("too-many-messages".equals(unsuitableType)) {
        ...
        for (int j = 0; j < messagesToPublish; j++) {
          allTasks.add(
              new com.amazon.mq.rabbitmq.publishing.PublishingTask(queueName, smallMessageBody));
        }
        logger.debug(
            "Generated {} small message tasks for queue: {}", messagesToPublish, queueName);
        ...
        for (int j = 0; j < messagesToPublish; j++) {
          allTasks.add(
              new com.amazon.mq.rabbitmq.publishing.PublishingTask(queueName, largeMessageBody));
        }
        logger.debug(
            "Generated {} large message tasks for queue: {}", messagesToPublish, queueName);
      }
      // "reject-publish-dlx" queues don't need to be filled with messages
    }

    logger.info(
        "Generated {} total publishing tasks for {} unsuitable queues",
        allTasks.size(),
        unsuitableQueueCount);
    return allTasks;
  }

  private void monitorAggregateProgress(PublishingResult result, int totalTasks)
    ...
    while (!result.isComplete()) {
      ...
      if (currentPublished - lastReported >= 5000 || currentPublished == totalTasks) {
        ...
        logger.info(
            "Unsuitable queues: {}/{} messages ({}%) - {} msg/sec",
  ...
  private void setupHAPolicies() throws Exception {
    ...
    try {
      ...
      getNextHttpClient()
          .declarePolicy(config.getAmqpEndpoint(0).getVirtualHost(), policy.getName(), policy);
      ...
      logger.info(
          "HA policy creation failed, but continuing with setup. Queues will not be mirrored.");
      logger.info("You can manually create the HA policy later using:");
      logger.info(
          "  rabbitmqctl set_policy test-ha-policy \"ha-all\""
    ...
    for (int i = 0; i < queueCount; i++) {
      ...
      while (attempt < maxAttempts && !queueSynchronized) {
        ...
        try {
          ...
          QueueInfo queueInfo =
              getNextHttpClient().getQueue(config.getAmqpEndpoint(0).getVirtualHost(), queueName);
          ...
          if (synchronizedSlaveNodes != null && synchronizedSlaveNodes.size() >= 2) {
            ...
            logger.debug(
                "Queue {} is fully synchronized with {} slaves (attempt {})",
                queueName,
                synchronizedSlaveNodes.size(),
                attempt);
          } else {
            logger.debug(
                "Queue {} has {} synchronized slaves (expected 2, attempt {})",
                queueName,
                synchronizedSlaveNodes != null ? synchronizedSlaveNodes.size() : 0,
                attempt);
          ...
          logger.warn(
              "Error checking HA synchronization status for queue {} (attempt {}): {}",
              queueName,
              attempt,
              e.getMessage());
          Thread.sleep(1000);
  ...
  private void createBindings() throws IOException {
    ...
    if (bindingsPerQueue > 0) {
      logger.info(
          "Creating bindings: {} queues × {} bindings per queue", queueCount, bindingsPerQueue);
    } else {
  ...
  private void publishMessages() throws IOException, InterruptedException {
    ...
    logger.info(
        "Publishing {} messages with confirmation window size: {}",
        totalMessages,
    ...
    logger.info(
        "Published {} messages in {} ms (avg {} msg/sec) using {} threads",
        globalPublishedCount.get(),
        elapsed,
        String.format("%.1f", avgRate),
        connectionCount);
  }
  ...
  private void monitorPublishingProgress(int totalMessages) throws InterruptedException {
    ...
    while (globalPublishedCount.get() < totalMessages) {
      ...
      if (currentPublished - lastReported >= 1000) {
        ...
        logger.info(
            "Published {}/{} messages ({} msg/sec) - Confirmed: {}",
            currentPublished,
            totalMessages,
    ...
    logger.info(
        "All {} messages published, waiting for threads to complete confirmations...",
        totalMessages);
  }
  ...
  private void verifySetup() throws Exception {
    ...
    try {
      ...
      List<QueueInfo> testQueues =
          queues.stream().filter(q -> q.getName().startsWith(config.getQueuePrefix())).toList();

      logger.info("Found {} test queues", testQueues.size());

      // Verify message counts
      long totalMessages = testQueues.stream().mapToLong(q -> q.getMessagesReady()).sum();
      ...
      long testBindings =
          bindings.stream()
              .filter(
                  b ->
                      b.getSource().startsWith(config.getExchangePrefix())
                          || b.getDestination().startsWith(config.getQueuePrefix()))
              .count();
  ...
  public Map<String, Object> getSetupStatistics() throws Exception {
    ...
    try {
      // Get the complete statistics (no need to wait for stabilization since we use publisher
      // confirms)
      List<QueueInfo> queues = getNextHttpClient().getQueues();
      List<QueueInfo> testQueues =
          queues.stream().filter(q -> q.getName().startsWith(config.getQueuePrefix())).toList();
