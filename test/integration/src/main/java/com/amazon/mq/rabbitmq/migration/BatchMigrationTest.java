...
/**
 * Tests batch migration functionality. Creates queues with varying sizes, then migrates in batches
...
public class BatchMigrationTest {
  ...
  private static String runBatchMigration(
      QueueMigrationClient client, int batchSize, String batchOrder) throws Exception {
  ...
  private static boolean validateBatchResult(
      TestConfiguration config, String migrationId, int expectedQuorum, int expectedClassic)
      throws Exception {
    boolean valid = true;
    ...
    if (details.getCompletedQueues() != BATCH_SIZE) {
      logger.error(
          "❌ Expected {} queues migrated, got {}", BATCH_SIZE, details.getCompletedQueues());
      valid = false;
    } else {
