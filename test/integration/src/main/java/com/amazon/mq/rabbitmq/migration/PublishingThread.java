...
public class PublishingThread implements Runnable {
    ...
    public PublishingThread(TestConfiguration config, int connectionIndex, BlockingQueue<PublishingTask> taskQueue,
            AtomicInteger globalPublishedCount, AtomicInteger globalConfirmedCount) {
        this.config = config;
    ...
    public void run() {
        try {
            ...
            if (failureCount < MAX_FAILURES) {
                failureCount++;
                logger.info("Restarting publishing thread {} (attempt {}/{})", connectionIndex, failureCount, MAX_FAILURES);
                run(); // Restart
            } else {
    ...
    private void processPublishingTasks() throws InterruptedException, IOException {
        ...
        logger.debug("Publishing thread {} finished processing {} tasks", connectionIndex, currentTaskIndex);
    }
    ...
    private void publishMessage(PublishingTask task) throws IOException, InterruptedException {
        ...
        Map<String, Object> headers = MessageGenerator.generateHeaders(task.messageNumber,
            task.messageSize.toString());
    ...
    private void handleConfirmation(long deliveryTag, boolean multiple, boolean ack) {
        synchronized (confirmationLock) {
            ...
            if (!ack) {
                logger.warn("Received NACK for delivery tag {} on connection {}", deliveryTag, connectionIndex);
            }
    ...
    private void waitForFinalConfirmations() throws InterruptedException {
        logger.debug("Publishing thread {} waiting for {} pending confirmations", connectionIndex, threadPendingConfirmations.size());
        ...
        if (!threadPendingConfirmations.isEmpty()) {
            logger.warn("Publishing thread {} timed out waiting for {} confirmations", connectionIndex, threadPendingConfirmations.size());
        }
    }
    ...
    void shutdown() {
        try {
            ...
            logger.warn("Error closing publishing thread {} resources: {}", connectionIndex, e.getMessage());
        }
    }
}
