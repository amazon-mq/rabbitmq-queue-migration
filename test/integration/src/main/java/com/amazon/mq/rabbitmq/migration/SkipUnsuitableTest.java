...
/**
 * Tests skip unsuitable queues functionality. Creates mix of suitable and unsuitable queues,
 * validates that: 1. Migration fails without skip flag 2. Migration succeeds with skip flag 3.
 * Unsuitable queues are skipped with correct reasons
 */
public class SkipUnsuitableTest {
  ...
  public static void main(String[] args) {
    try {
      ...
      logger.info(
          "=== Phase 1: Setup ({} suitable, {} unsuitable queues) ===",
  ...
  private static boolean validateResults(
      QueueMigrationClient client, TestConfiguration config, String migrationId) throws Exception {
    boolean valid = true;
    ...
    if (skippedWithUnsuitableReason != UNSUITABLE_QUEUE_COUNT) {
      logger.error(
          "❌ Expected all {} skipped queues to have 'unsuitable_overflow' reason, got {}",
          UNSUITABLE_QUEUE_COUNT,
          skippedWithUnsuitableReason);
      valid = false;
    } else {
    ...
    for (QueueInfo queue : queues) {
      if (queue.getName().startsWith("test.queue.")
          || queue.getName().startsWith("test.unsuitable.queue.")) {
    ...
    if (classicCount != UNSUITABLE_QUEUE_COUNT) {
      logger.error(
          "❌ Expected {} classic queues remaining, got {}", UNSUITABLE_QUEUE_COUNT, classicCount);
      valid = false;
    } else {
