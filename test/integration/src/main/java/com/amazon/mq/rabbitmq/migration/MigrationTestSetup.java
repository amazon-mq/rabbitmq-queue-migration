 ...
 *
 * <p>Usage: java -jar migration-test-setup.jar <instance-type> [options]
 *
 * <p>Example: java -jar migration-test-setup.jar m5.xlarge
 */
public class MigrationTestSetup {
  ...
  private static void printUsage() {
    ...
    System.out.println(
        "  --queue-count=N            Number of queues to create (min: 10, max: 65536, default:"
            + " 10)");
    System.out.println(
        "  --total-messages=N         Total messages across all queues (default: 5500)");
    ...
    System.out.println(
        "  --confirmation-window=N    Confirmations per publishing thread (min: 4, max: 256,"
            + " default: 4)");
    System.out.println(
        "  --migration-timeout=N      Migration timeout in seconds (default: 300, for end-to-end"
            + " mode)");
    System.out.println(
        "  --unsuitable-queue-count=N  Number of unsuitable queues to create for testing (default:"
            + " 0)");
    System.out.println(
        "                                Creates queues with reject-publish-dlx, too many messages,"
            + " etc.");
    System.out.println(
        "  --queue-prefix=PREFIX      Prefix for queue names (default: test.queue.ha-all.)");
    System.out.println(
        "  --unsuitable-queue-prefix=PREFIX  Prefix for unsuitable queue names (default:"
            + " test.unsuitable.queue.ha-all.)");
    System.out.println(
        "  --exchange-prefix=PREFIX   Prefix for exchange names (default: test.exchange.)");
    System.out.println();
    System.out.println("Message Configuration:");
    System.out.println(
        "  --message-distribution=X,Y,Z   Message size distribution percentages (must sum to 100)");
    System.out.println("                                 Example: --message-distribution=70,20,10");
    System.out.println(
        "  --message-sizes=S,M,L          Message sizes in bytes (min: 8, max: 4MiB)");
    System.out.println(
        "                                 Example: --message-sizes=1024,102400,1048576");
    System.out.println();
    ...
    System.out.println(
        "  --batch-order=ORDER        Order to migrate queues: smallest_first or largest_first");
    System.out.println("  --enable-ttl               Enable TTL on queues (disabled by default)");
    System.out.println(
        "  --ttl-hours=HOURS          Set TTL duration in hours (default: 1, enables TTL)");
    System.out.println(
    ...
    System.out.println(
        "  java -jar migration-test-setup.jar setup-env --queue-count=20 --total-messages=10000");
    System.out.println();
    ...
    System.out.println(
        "  java -jar migration-test-setup.jar end-to-end --queue-count=10 --migration-timeout=600");
    System.out.println();
    System.out.println("  # End-to-end test without cleanup or setup");
    System.out.println(
        "  java -jar migration-test-setup.jar end-to-end --queue-count=10 --skip-cleanup"
            + " --skip-setup");
    System.out.println();
  }

  private static void printSetupSummary(
      Map<String, Object> stats, long setupTimeMs, TestConfiguration config) {
    System.out.println();
  ...
  private static void ensureVhostExists(ClusterTopology topology, String vhost) {
    try {
      ...
      boolean vhostExists = vhosts.stream().anyMatch(v -> vhost.equals(v.getName()));
