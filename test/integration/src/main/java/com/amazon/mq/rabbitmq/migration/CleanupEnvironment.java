...
/**
 * Aggressive cleanup of RabbitMQ test environment. Matches the behavior of
 * cleanup_previous_test_artifacts() in the shell script.
 */
public class CleanupEnvironment {
  ...
  private static void checkMigrationRecords(QueueMigrationClient migrationClient) {
    ...
    try {
      ...
      logger.info(
          "Could not check migration records (migration plugin may not be available): {}",
          e.getMessage());
    }
  }
