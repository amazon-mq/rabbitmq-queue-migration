...
/**
 * Tests migration interruption functionality. Starts a migration, interrupts after a threshold of
 * completed queues, and validates the interrupted state.
 */
public class InterruptionTest {
  ...
  private static boolean waitAndInterrupt(
      QueueMigrationClient client, String migrationId, int threshold) throws Exception {
    ...
    while (System.currentTimeMillis() - startTime < timeout) {
      ...
      if (completedCount == 0 && !details.getQueueStatuses().isEmpty()) {
        completedCount =
            (int)
                details.getQueueStatuses().stream()
                    .filter(q -> "completed".equals(q.getStatus()))
                    .count();
        logger.info(
            "Counted {} completed queues from {} queue details",
            completedCount,
            details.getQueueStatuses().size());
      }
      ...
      if (details.isCompleted() || details.isInterrupted()) {
        logger.error(
            "Migration finished before we could interrupt (completed {} queues)", completedCount);
        return false;
      }
    ...
    while (System.currentTimeMillis() - startTime < timeout) {
      ...
      if (!details.isInProgress()) {
        ...
        while (System.currentTimeMillis() - finalizeStart < finalizeTimeout) {
          ...
          logger.info(
              "Waiting for queue statuses to finalize: {} completed + {} skipped = {} of {} total",
          ...
          if (accountedFor == details.getTotalQueues()) {
            logger.info(
                "All queue statuses finalized: {} completed + {} skipped = {} total",
                details.getCompletedQueues(),
                skippedCount,
                details.getTotalQueues());
            return;
  ...
  private static boolean validateInterruption(
      QueueMigrationClient client,
      TestConfiguration config,
      String migrationId,
      Map<String, Long> preMigrationCounts)
      throws Exception {
    boolean valid = true;
    ...
    if (completedCount < INTERRUPT_AFTER_QUEUES) {
      logger.error(
          "❌ Expected at least {} completed queues, got {}",
          INTERRUPT_AFTER_QUEUES,
          completedCount);
      valid = false;
    } else {
    ...
    if (skippedWithInterruptReason != skippedCount) {
      logger.error(
          "❌ Expected all skipped queues to have reason 'interrupted', got {}/{}",
          skippedWithInterruptReason,
          skippedCount);
      valid = false;
    } else {
    ...
    if (totalQueues != QUEUE_COUNT) {
      logger.error(
          "❌ Total queue count ({}) doesn't match expected ({})", totalQueues, QUEUE_COUNT);
      valid = false;
    } else {
    ...
    if (completedCount + skippedCount != QUEUE_COUNT) {
      logger.error(
          "❌ Completed ({}) + skipped ({}) doesn't equal total ({})",
          completedCount,
          skippedCount,
          QUEUE_COUNT);
      valid = false;
