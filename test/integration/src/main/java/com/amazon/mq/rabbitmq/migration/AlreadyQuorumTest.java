...
/**
 * Tests migration behavior when some queues are already quorum type.
 * Validates idempotency - already-converted queues should be skipped.
 */
public class AlreadyQuorumTest {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Phase 1: Setup
            logger.info("=== Phase 1: Setup ({} classic + {} quorum queues, {} messages) ===",
                CLASSIC_QUEUE_COUNT, QUORUM_QUEUE_COUNT, TOTAL_MESSAGES);
    ...
    private static void verifyInitialState(TestConfiguration config) throws Exception {
        ...
        if (classicCount != CLASSIC_QUEUE_COUNT) {
            throw new RuntimeException(
                String.format("Expected %d classic queues, found: %d", CLASSIC_QUEUE_COUNT, classicCount));
        }
    ...
    private static void validateResults(QueueMigrationClient client, String migrationId,
        ...
        if (details.getTotalQueues() != CLASSIC_QUEUE_COUNT) {
            throw new RuntimeException(
                String.format("Expected %d total queues, got: %d",
                    CLASSIC_QUEUE_COUNT, details.getTotalQueues()));
        }
        ...
        if (classicCount != 0) {
            throw new RuntimeException(
                String.format("Expected 0 classic queues, got: %d", classicCount));
        }
    }
}
