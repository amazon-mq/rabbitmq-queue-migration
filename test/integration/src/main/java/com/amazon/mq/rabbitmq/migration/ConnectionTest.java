...
public class ConnectionTest {
    ...
    public static boolean testAmqpConnection(TestConfiguration config) {
        ...
        for (int i = 0; i < config.getClusterTopology().getNodeCount(); i++) {
            ...
            logger.info("Testing AMQP connection to {}:{} vhost '{}'",
            ...
            try (Connection connection = factory.newConnection("connection-test")) {
                logger.info("✅ AMQP connection successful to {}:{}", endpoint.getHostname(), endpoint.getPort());
            } catch (Exception e) {
                logger.error("❌ AMQP connection failed to {}:{}: {}", endpoint.getHostname(), endpoint.getPort(), e.getMessage());
                allSuccessful = false;
                ...
                if (e.getCause() instanceof com.rabbitmq.client.ShutdownSignalException) {
                    ...
                    if (reason.contains("vhost") && reason.contains("not found")) {
                        ...
                        logger.info("Grant permissions with: rabbitmqctl set_permissions -p {} {} \".*\" \".*\" \".*\"",
                            endpoint.getVirtualHost(), endpoint.getUsername());
                    } else if (reason.contains("ACCESS_REFUSED")) {
                        logger.error("Authentication failed for user '{}'", endpoint.getUsername());
                        logger.info("Check username/password or create user with: rabbitmqctl add_user {} <password>",
                            endpoint.getUsername());
                        logger.info("Set admin tag with: rabbitmqctl set_user_tags {} administrator",
                            endpoint.getUsername());
                    }
                }
            }
    ...
    public static boolean testManagementConnection(TestConfiguration config) {
        ...
        for (int i = 0; i < config.getClusterTopology().getNodeCount(); i++) {
            ...
            try {
                ...
                logger.error("❌ Management API connection failed to {}: {}", client.toString(), e.getMessage());

                String errorMsg = e.getMessage().toLowerCase();
                if (errorMsg.contains("401") || errorMsg.contains("unauthorized")) {
                    logger.error("Authentication failed for management API");
                    logger.info("Ensure user 'guest' has administrator tag: rabbitmqctl set_user_tags guest administrator");
