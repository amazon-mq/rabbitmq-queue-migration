  ...
  private static void validateResults(QueueMigrationClient client, String migrationId)
    ...
    if (details.getCompletedQueues() != QUEUE_COUNT) {
      throw new RuntimeException(
          String.format(
              "Expected %d completed queues, got: %d", QUEUE_COUNT, details.getCompletedQueues()));
    }

    if (details.getTotalQueues() != QUEUE_COUNT) {
      throw new RuntimeException(
          String.format(
              "Expected %d total queues, got: %d", QUEUE_COUNT, details.getTotalQueues()));
    }
    ...
    if (skippedCount != 0) {
      throw new RuntimeException(String.format("Expected 0 skipped queues, got: %d", skippedCount));
    }
  }
}
