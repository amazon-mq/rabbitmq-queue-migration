...
/**
 * Utility class for RabbitMQ statistics operations. Provides simple stats stabilization for
...
public class RabbitMQStatsUtils {
  ...
  /**
   * Wait for test queue message counts to stabilize. This method polls RabbitMQ queue statistics
   * and waits until the message count remains exactly the same between consecutive checks.
   *
   ...
   */
  public static long waitForTestQueueStatsToStabilize(
      Client httpClient, String vhost, String queuePrefix, String context) throws Exception {
    logger.info(
        "Waiting for RabbitMQ stats to stabilize{}...",
    ...
    while (attempt <= maxAttempts) {
      ...
      try {
        ...
        if (currentCount == previousCount && attempt > 1) {
          logger.info(
              "✅ Stats have stabilized at {} messages after {} seconds",
              currentCount,
              attempt * sleepDurationSeconds);
          return currentCount;
        }
    ...
    logger.warn(
        "Stats stability timeout reached after {} seconds", maxAttempts * sleepDurationSeconds);
    return currentCount;
  }
}
