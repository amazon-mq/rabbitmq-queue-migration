...
handle_call(
    ...
    case queue:out(Values) of
        ...
        {empty, _} ->
            {noreply, State#gstate{blocked = queue:in(From, Blocked)}, hibernate};
        {{value, {PendingIn, Value}}, NewValues} ->
            reply(PendingIn, ok),
            {reply, {value, Value}, State#gstate{values = NewValues}, hibernate}
    end;
...
handle_cast(finish, State = #gstate{forks = Forks, blocked = Blocked}) ->
    ...
    {noreply, State#gstate{forks = NewForks, blocked = NewBlocked}, hibernate};
